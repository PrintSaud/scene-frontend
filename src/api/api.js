...
if (!BASE_URL) {
  throw new Error(
    "EXPO_PUBLIC_BACKEND_URL is missing! Add it to your Expo env before building."
  );
}

console.log("🔥 Mobile backend env:", process.env.EXPO_PUBLIC_BACKEND_URL);
...
export const addLogReply = (logId, data) =>
  api.post(`/api/logs/${logId}/reply`, data);
export const reactToLog = (logId, emoji) =>
  api.post(`/api/logs/${logId}/react`, { emoji });
export const deleteReply = (logId, replyId) =>
  api.delete(`/api/logs/${logId}/replies/${replyId}`);
...
// Likes for replies
//
export const likeReply = (logId, replyId) =>
  api.post(`/api/logs/${logId}/replies/${replyId}/like`);
...
export const toggleWatchlist = (movieId) =>
  api.post(`/api/watchlist/toggle`, { movieId });
...
export const votePoll = (pollId, choice) =>
  api.post(`/api/polls/${pollId}/vote`, { choice });
...
export const markAllNotificationsRead = () =>
  api.patch("/api/notifications/markAll");
...
export const followUser = (userId, targetId) =>
  api.post(`/api/users/${userId}/follow/${targetId}`);
